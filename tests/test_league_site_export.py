...
_pilot_results_rows now empties it out instead -- the page's existing r.deck || "-" fallback then
renders it exactly like any other missing deck value, with no internal pipeline status ("Unknown",
"pending review", ...) surfaced to a reader who doesn't need it.
...
def test_needs_manual_review_deck_is_emptied_out() -> None:
    grp = pd.DataFrame([
        {
    ...
    assert by_event["12854081"]["deck"] == "", (
        "an unresolved deck must be emptied out, not show the raw NEEDS_MANUAL_REVIEW marker "
        "(or any other internal-status label a reader doesn't need)"
    )
    assert by_event["12853780"]["deck"] == "Esper Blink", "a real deck name must pass through unchanged"


if __name__ == "__main__":
    test_needs_manual_review_deck_is_emptied_out()
    print("OK: test_needs_manual_review_deck_is_emptied_out")
